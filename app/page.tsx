...
export default function HomePage() {
  ...
  const { projects, isLoading, error, opfsSupported } = useSelector(appStore, (state) => state);
  ...
  const handleUpload = async (file: File) => {
    try {
      ...
      for (const epubFile of epubStructure.files) {
        await opfs.writeFile(projectId, epubFile.path, epubFile.content);
      }
      ...
      await opfs.writeFile(projectId, 'metadata.json', JSON.stringify(projectMetadata, null, 2));
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to upload EPUB');
      throw err;
    } finally {
      appStore.send({ type: 'setLoading', loading: false });
    }
        ...
        <Card className="max-w-md">
          ...
          <CardContent>
            <Alert variant="destructive">
              <AlertDescription>
                {error || 'Your browser does not support the required features for this application.'}
              </AlertDescription>
            </Alert>
            <p className="text-sm text-muted-foreground mt-4">
              Please use a modern browser like Chrome, Edge, or Firefox to access the EPUB editor.
            </p>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="max-w-md mx-auto">
            <Card className="text-center">
              <CardHeader>
                ...
                <CardDescription>
                  Upload an EPUB file to create your first project
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Button onClick={() => setUploadDialogOpen(true)} size="lg" className="w-full">
