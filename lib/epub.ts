    ...
    return new Promise((resolve, reject) => {
      parseString(opfContent, (err, result) => {
        ...
        try {
          ...
          // Extract manifest
          const manifest: EpubManifest[] = pkg.manifest[0].item.map((item: any) => ({
            id: item.$.id,
            href: item.$.href,
          ...
          // Extract spine
          const spine: EpubSpine[] = pkg.spine[0].itemref.map((itemref: any) => ({
            idref: itemref.$.idref
          }));
...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
  <rootfiles>
    ...
    for (const file of structure.files) {
      if (!file.path.startsWith('META-INF/') && file.path !== 'mimetype') {
