export class OPFSManager {
  ...
  async getProject(projectId: string): Promise<FileSystemDirectoryHandle | null> {
    if (!this.root) throw new Error('OPFS not initialized');
  ...
  async writeFile(projectId: string, filePath: string, content: string | ArrayBuffer): Promise<void> {
  ...
  async listFiles(projectId: string, dirPath: string = ''): Promise<{ name: string; type: 'file' | 'directory' }[]> {
    const projectDir = await this.getProject(projectId);
    if (!projectDir) throw new Error('Project not found');
